...
def main(cloud_event, abc):
    ...
    try:
        ...
        existing_ids = bigquery_handler.fetch_existing_ids_from_bigquery(
            "res_partner")
        ...
        category_records = odoo_api.fetch_res_partner_category()
        mapped_new_records = odoo_api.map_category_ids_with_names(
            new_records, category_records)

        if mapped_new_records:
            # Step 3: Insert only new records into BigQuery
            bigquery_handler.insert_into_bigquery(
                "res_partner", mapped_new_records, 1000)
            logging.info(
                f"Successfully inserted {len(mapped_new_records)} new records into BigQuery table 'res_partner'.")
        else:
            logging.info("No new res_partner to insert.")

    except Exception as e:
        logging.error(
            f"Error in fetching or loading res_partner to BigQuery: {e}")
