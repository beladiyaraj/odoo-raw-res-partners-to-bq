...
class OdooAPI:
    ...
    def fetch_res_partner(self, existing_ids):
        ...
        fields = ["id", "name", "state_id", "street", "street2", "contact_address_complete", "contact_type"]  #, "invoice_user_id"]

        # Fetch data from Odoo
        records = self._make_request('res.partner', fields)
        logging.info(f"Data retrieved from API. Total records: {len(records)}.")
        if not records:
