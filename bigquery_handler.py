...
class BigQueryHandler:
    ...
    def fetch_existing_ids_from_bigquery(self, table_name):
        ...
        try:
            ...
            logging.info(f"Table {table_name} does not exist in {self.dataset_id}. {e}")
            return []
        ...
        logging.info(f"Fetched {len(existing_ids)} existing IDs from BigQuery table {table_name}.")
        return existing_ids
    ...
    def upload_to_gcs(self, data, gcs_path):
        ...
        try:
            ...
            logging.info(f"Successfully uploaded rows {len(data)} to {gcs_path} in GCS bucket {self.bucket_name}.")
        except Exception as e:
    ...
    def insert_into_bigquery(self, table_name, data, chunk_size=500):
        ...
        try:
            ...
            try:
                ...
                logging.info(f"Created table {table_name} in BigQuery with specified schema.")

            # Insert data in chunks
            total_records = len(data)
            logging.info(f"Inserting {total_records} records into BigQuery table {table_name} in chunks.")
            ...
            for i in range(0, total_records, chunk_size):
                ...
                if errors:
                    logging.error(f"Errors occurred while inserting rows into BigQuery: {errors}")
                else:
                    logging.info(f"Successfully inserted batch {i // chunk_size + 1} of total {total_batches} batches.")
